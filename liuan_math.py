...
def gao_tao_ai_engine(sys_msg, user_msg, api_key, is_review=False, is_json=False):
    ...
    if is_review:
        ...
        base_instruction = (
            ...
            "2. 严禁重复题干内容，严禁给出完整解析过程。\n"
            "3. 只给出‘题眼’引导学生思考。语气要极其温柔亲切。\n"
            "4. 严禁使用 LaTeX，数学关系请用汉字描述。"
        )
    else:
        # 🌟 命题模式提示词：只给题干选项
        base_instruction = "你现在是李鹏燕老师。任务：命制单选题。要求：只提供题干和选项，绝对严禁提供答案和任何解析。严禁 LaTeX。"
...
def process_full_paper(file, api_key):
    try:
        ...
        full_raw_text = "\n".join(stream)
        sys_prompt = "任务：拆解23道题。要求：识别锚点URL，公式转汉字，严格JSON。"
...
def get_question(m_cat, s_cat, api_key):
    ...
    if res.data:
        ...
        # 🌟 此处强调：不要解析
        prompt = f"出一道【{s_cat}】考点的单选题。只给题干选项，严禁给答案和解析。"
        return gao_tao_ai_engine("命题专家", prompt, api_key, is_review=False), False
    ...
    try:
        ...
        with st.expander("📂 Word一键图文识别入库", expanded=True):
            ...
            if word_file and st.button("🚀 执行多模态识别"):
                with st.status("🔍 正在物理解析...", expanded=True) as status:
...
if "curr_student" in locals() and curr_student:
    ...
    with c2: st.markdown(f'<div class="metric-card"><h3>🎯 攻坚</h3><h2 style="color:#D32F2F;">{active_kps[scores.index(min(scores))]}</h2></div>', unsafe_allow_html=True)
   ...
   with tab4:
        ...
        if check_res.data:
            for q_item in check_res.data:
                ...
                if q_item.get('image_url'): st.image(q_item['image_url'], width=300)
                st.divider()
